...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/SGSModV3/wiki/Home) | [English Home](https://github.com/alkaid616/SGSModV3/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `SGSModV3`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `SGSModV3.json` and update `id`, `name`, `author`, and `description`.
3. Edit `SGSModV3Code/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `SGSModV3/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `SGSModV3.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `SGSModV3.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\SGSModV3.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/SGSModV3` directory.
...
```text
SGSModV3/
├── SGSModV3Code/   # C# source
├── SGSModV3/       # Godot resources, localization, and placeholder scenes
├── SGSModV3.csproj
...
`res://SGSModV3/...` is the Godot/PCK resource path, mapping to the repository resource directory `SGSModV3/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `SGSModV3Character` |
| Expected ID | `SGS_MOD_V3_CHARACTER_SGS_MOD_V3_CHARACTER` |
| Starter deck | 4 × `SGSModV3Strike`, 4 × `SGSModV3Defend`, 1 × `SGSModV3Relic` |
...
|---|---|---|
| `SGSModV3Strike` (attack) | character card pool | `SGS_MOD_V3_CARD_SGS_MOD_V3_STRIKE` |
| `SGSModV3Defend` (skill) | character card pool | `SGS_MOD_V3_CARD_SGS_MOD_V3_DEFEND` |
| `SGSModV3Relic` | `SGSModV3RelicPool` | `SGS_MOD_V3_RELIC_SGS_MOD_V3_RELIC` |

### Static placeholder assets

**Images** (`res://SGSModV3/images/...`):

- `cards/SGSModV3Strike.png`, `cards/SGSModV3Defend.png`: sample card art.
- `relics/SGSModV3Relic.png`: sample relic icon.
- `characters/SGSModV3_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://SGSModV3/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `SGSModV3_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `SGSModV3_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `SGSModV3_merchant.tscn` | Merchant | — |
| `SGSModV3_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `SGSModV3_character_select_bg.tscn` | Character select background | — |
...
`SGSModV3.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
