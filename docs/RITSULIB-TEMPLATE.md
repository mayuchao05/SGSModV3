...
- 模板 Wiki（以 Rider 为主线）：[中文首页](https://github.com/alkaid616/SGSModV3/wiki/Home) | [English Home](https://github.com/alkaid616/SGSModV3/wiki/Home-EN)。
...
`dotnet new ritsulibmod -n MyMod` 会生成名为 `MyMod` 的工程，并把模板中的 `SGSModV3`、示例类名、资源文件名、资源目录、manifest 名称、namespace 和本地化 id 同步替换成新名称。

### 方式 B：手动复制模板

1. 复制整个目录并改名为你的 Mod 名称。
2. 修改 `SGSModV3.json` 里的 `id`、`name`、`author`、`description`。
3. 修改 `SGSModV3Code/Entry.cs` 里的 `ModId`。
4. 如需彻底改名，同时修改 `.csproj`、`.sln`、`project.godot` 的项目名和命名空间。
5. 把资源目录 `SGSModV3/` 改成你的 `ModId`，并同步更新代码中的 `Entry.ResPath` 相关路径。
...
>
> `SGSModV3.json` 中 `dependencies[STS2-RitsuLib].version` **必须**与 `.csproj` 里 `STS2.RitsuLib` 包实际编译使用的版本一致。模板构建时会自动同步该依赖版本；`min_game_version` 和有意声明较低运行时下限的场景仍需人工确认。详细步骤见下方 [发布前 checklist：版本对齐](#发布前-checklist版本对齐)。
...
> **`.csproj` 里的 `PackageReference` 只控制编译时拉取；`SGSModV3.json` 的 `dependencies` 是游戏加载器在运行时校验的。模板会在构建时把 `STS2-RitsuLib` 依赖版本同步为实际解析到的 NuGet 版本，但 `min_game_version` 仍需人工确认。**
...
1. 构建后确认 `SGSModV3.json` 的 `dependencies[STS2-RitsuLib].version` 已同步为实际解析到的 `STS2.RitsuLib` 版本。
...
| 命令 | 行为 |
|---|---|
| `dotnet build .\SGSModV3.csproj` | 完整构建：编译 + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**：复制 dll 和 manifest 到游戏的 `mods/SGSModV3` 目录。
...
`res://SGSModV3/...` 是 Godot/PCK 内的资源路径，对应仓库里的 `SGSModV3/` 资源目录，**不是 C# namespace**。通过 NuGet 模板创建项目时，这些目录名、文件名和 namespace 会按新 Mod 名同步替换。
...
| 项 | 值 |
|---|---|
| 类型 | `SGSModV3Character` |
| 预期 id | `SGS_MOD_V3_CHARACTER_SGS_MOD_V3_CHARACTER` |
| starter 牌组 | 4 × `SGSModV3Strike`、4 × `SGSModV3Defend`、1 × `SGSModV3Relic` |
...
| 类型 | 池 | 预期 id |
|---|---|---|
| `SGSModV3Strike`（攻击） | 角色卡池 | `SGS_MOD_V3_CARD_SGS_MOD_V3_STRIKE` |
| `SGSModV3Defend`（技能） | 角色卡池 | `SGS_MOD_V3_CARD_SGS_MOD_V3_DEFEND` |
| `SGSModV3Relic` | `SGSModV3RelicPool` | `SGS_MOD_V3_RELIC_SGS_MOD_V3_RELIC` |

### 静态占位资源

**图片**（`res://SGSModV3/images/...`）：

- `cards/SGSModV3Strike.png`、`cards/SGSModV3Defend.png`：示例卡图。
- `relics/SGSModV3Relic.png`：示例遗物图标。
- `characters/SGSModV3_character_*.png`：角色头像、角色选择图、地图标记和能量图标。

**场景**（`res://SGSModV3/scenes/characters/...`）：

| 场景文件 | 用途 | 占位结构 |
|---|---|---|
| `SGSModV3_character.tscn` | 战斗人物 | `%Visuals`、`%Bounds`、`%IntentPos`、`%CenterPos`、`%TalkPos` |
| `SGSModV3_energy_counter.tscn` | 能量表盘 | `%EnergyVfxBack`、`%Layers`、`%RotationLayers`、`%EnergyVfxFront`、`Label` |
| `SGSModV3_merchant.tscn` | 商店人物 | — |
| `SGSModV3_rest_site.tscn` | 火堆人物 | `%ControlRoot`、`%SelectionReticle`、`%Hitbox`、`%ThoughtBubbleRight`、`%ThoughtBubbleLeft` |
| `SGSModV3_character_select_bg.tscn` | 角色选择背景 | — |

这些资源只用于保证模板可见、可替换，不追求原版动画效果。复制模板后替换为自己的素材即可；如果改了路径，同步更新对应 `AssetProfile`。

## Manifest 格式

`SGSModV3.json` 是 Mod 的清单文件，游戏加载器在启动时读取它来识别 Mod、检查依赖、决定是否加载。完整示例：
